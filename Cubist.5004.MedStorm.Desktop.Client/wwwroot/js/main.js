var app = (function () {
    ...
    let noOfSkinCondData = 17;  // 17 samples with 1 sec. sampling, i.e. roghly 20 sec. timedisplay
    let noOfData = 62;          // 62 samples with 1 sec. sampling, i.e. roghly 1 min timedisplay
    let badSignalData = [];
    let patientId;
            ...
            incomingObject.properties.forEach(async prop => {
                if (prop.id != 'skincond') {
                    if (applications[selectedApplication].includes(prop.id)) {
                        ...
                        graphData[prop.id].push({ timestamp: incomingObject.timestamp, value: prop.value });
                        while (graphData[prop.id].length > noOfData) {
                            graphData[prop.id].shift();
                        }
                    }

                    fullDataSet[prop.id].push({ timestamp: incomingObject.timestamp, value: prop.value });
                    while (fullDataSet[prop.id].length > noOfSkinCondData) {
    ...
    function addSkinCondData(skinCondArray, timestampIncomingObject) {
        skinCondGraphData.push({ timestamp: timestampIncomingObject, value: skinCondArray[0] });
        while (skinCondGraphData.length > noOfData) {
